...
iteraciones_de_k = 200; #cuantas veces vamos a probar ajustar con cada k el mismo set de datos
maximas_iteraciones_de_k_means = 100; #Maximo de iteraciones que hace el kmeans si no converge
...
#puntos_de_ruido <- matrix(runif(total_de_puntos*2, 0, parametro_de_red*dim_red), nrow=total_de_puntos, ncol=2);
...
mejor_k <- matrix(0, nrow=10, ncol=2); #Vamos a guardar el mejor k y el promedio que dio
...
for(i in seq(from = 1, to = 10, by = 1)){
	...
	puntos <- rbind(puntos, matrix(runif(12*2, 0, parametro_de_red*dim_red), nrow=12, ncol=2));
